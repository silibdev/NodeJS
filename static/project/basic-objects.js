...
function uvCube2(side) {
   ...
   function face(xyz, nrm) {
      ...
      if(alt) texCoords.push(0,0,1,0,1,1,0,1);
      else texCoords.push(0,0,0,1,1,1,1,0);
      alt=!alt;
      indices.push(start,start+1,start+2,start,start+2,start+3);
   }
   face( [-s,-s,s, s,-s,s, s,s,s, -s,s,s], [0,0,1] );
...
function conoTroncato(radius, radius2, height, slices, noTop, noBottom) {
   radius = radius || 0.5;
   height = height || 2*radius;
   slices = slices || 32;
   var vertexCount = 2*(slices+1);
   ...
   for (i = 0; i <= slices; i++) {
      ...
      texCoords[kt++] = i/slices;
      texCoords[kt++] = 0;
      vertices[kv] = c*radius2;
      normals[kv++] = c;
      vertices[kv] = s*radius2;
      normals[kv++] = s;
      vertices[kv] = height/2;
      normals[kv++] = 0;
      texCoords[kt++] = i/slices;
